...
void MQTT_Publish_Data(void)
{
    ...
    if (n > 0 && n < (int)sizeof(payload))
    {
        ...
        {
            HAL_UART_Transmit(&huart2, (uint8_t*)"Publish failed, retry...\r\n", 25, 100);
            delay_ms(200);

            if (!ESP8266_MQTT_Publish(MQTT_TOPIC_POST, payload, 0, 0))
            {
                HAL_UART_Transmit(&huart2, (uint8_t*)"Publish retry failed, reconnect MQTT\r\n", 35, 100);
                if (ESP8266_ConnectCloud())
                    HAL_UART_Transmit(&huart2, (uint8_t*)"MQTT reconnect OK\r\n", 19, 100);
            }
            else
            {
                msg_id++;
